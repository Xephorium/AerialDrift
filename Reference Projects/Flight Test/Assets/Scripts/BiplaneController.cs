...
public class BiplaneController : MonoBehaviour {
    ...
    private static float TRAIL_SPEED = 27f;
    private static float TRAIL_FADE = 5f;
    private static float MAX_GRAVITY = -13f;
    private static float MAX_FIN_ANGLE = 5f;
    ...
    private static float MAX_ENGINE_POWER = 37f;       // The maximum output of the engine.
    private static float LIFT_FACTOR = 0.004f;         // The amount of lift generated by the aeroplane moving forwards.
    ...
    private static float AERODYNAMICS_FACTOR = 0.02f;  // How much aerodynamics affect the speed of the aeroplane.
    private static float THROTTLE_CHANGE_RATE = 0.3f;  // The speed with which the throttle changes.
    private static float DRAG_FALLOFF_FACTOR = 0.001f; // How much drag should increase with speed.
    ...
    void FixedUpdate() {
        if (isPlayerControlling) {
            ...
            // Calculate Angles
            float horizontalFinAngle = MAX_FIN_ANGLE * -mousePercentY * 9;
            Mathf.Clamp(horizontalFinAngle, -MAX_FIN_ANGLE, MAX_FIN_ANGLE);
            float verticalFinAngle = (MAX_FIN_ANGLE / 3) * mousePercentX * 9;
